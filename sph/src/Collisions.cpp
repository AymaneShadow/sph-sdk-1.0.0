                                                    ...
                                                    SPHAlgorithms::Point3D differenceParticleNeighbour)
{
    ...
    if(particleDistance != 0)
        ...
        return particlePosition + (differenceParticleNeighbour) * Config::ParticleRadius;
}
...
static SPHAlgorithms::Point3D calculateSurfaceNormal(SPHAlgorithms::Point3D differenceParticleNeighbour)
{
    ...
    if (particleDistance != 0)
        ...
        return -differenceParticleNeighbour;
}
...
{
    //std::cout << std::endl << "particleVect.size() = " << particleVect.size() << std::endl;

    for (size_t i = 0; i < particleVect.size(); i++)
    {
        /* Particle Collision */

        //std::cout << std::endl << "particleVect[i].neighbours.size() = " << particleVect[i].neighbours.size() << std::endl;
        ...
        for (size_t j = 0; j < particleVect[i].neighbours.size(); j++)
        {
            ...
            if (calculateF(differenceParticleNeighbour) < 0)
            {
                ...
                //std::cout << std::endl << "particleVect[i].position = " << particleVect[i].position.x << std::endl;
